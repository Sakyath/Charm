  ...
  return (
    <div>
      <div className="px-5 pt-8 md:px-10">
        <button onClick={() => navigate({ to: "/shop" })} className="flex items-center gap-2 font-sans text-[0.6rem] uppercase tracking-[0.25em] text-ivory/55 hover:text-ivory">
      ...
      <div className="mx-auto grid max-w-[1400px] gap-12 px-5 py-10 md:grid-cols-2 md:px-10 md:py-16 md:gap-16">
        ...
        <div className="flex flex-col justify-center">
          ...
          <p className="mt-3 font-sans text-xl font-light text-gold">{formatPrice(product.price)}</p>
          <p className="mt-6 body-lg text-stone">{product.description}</p>

          <ul className="mt-8 space-y-2">
            {product.details.map((d) => (
              <li key={d} className="flex items-center gap-3 font-sans text-[0.78rem] font-light text-gold">
          ...
          <div className="mt-8 flex gap-3">
            <button type="button" onClick={() => { add(product.slug); toast.success("Added to your bag", { description: product.name }); }} className="glass-button flex flex-1 items-center justify-center gap-2 btn-gold py-4 text-center font-sans text-[0.65rem] uppercase tracking-[0.3em]">
              <ShoppingBag className="h-4 w-4" strokeWidth={1.4} /> Add to bag
            </button>
            <a href={createWhatsAppOrderLink(product)} target="_blank" rel="noreferrer" className="flex-1 btn-gold py-4 text-center font-sans text-[0.65rem] uppercase tracking-[0.3em]">
              Order Now
            </a>
          ...
          <div className="mx-auto mt-10 grid max-w-[1400px] grid-cols-2 gap-x-5 gap-y-10 md:grid-cols-4 md:gap-x-8">
            {related.map((p) => (
              <Link key={p.slug} to="/product/$slug" params={{ slug: p.slug }} className="group block">
                ...
                <p className="font-sans text-sm font-light text-gold">{formatPrice(p.price)}</p>
              </Link>
            ))}
          </div>
        </section>

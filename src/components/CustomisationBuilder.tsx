...
export default function CustomisationBuilder() {
  ...
  const chosen = useMemo(() => charmOptions.filter((c) => selected.includes(c.id)), [selected]);
    ...
    <div className="grid gap-12 lg:grid-cols-[1.35fr_1fr] lg:gap-20">
      <div>
        ...
        <p className="mt-3 max-w-lg text-sm leading-relaxed text-muted-foreground">
          Pick as many as feel right. Three to five is our favourite balance, but there are no rules
          here.
        </p>
      ...
      <aside className="h-fit border border-border bg-cream p-7 md:p-9 lg:sticky lg:top-28">
        ...
        <p className="mt-4 text-xs leading-relaxed text-muted-foreground">
          An estimate, not a checkout. We confirm the final piece, sizing and delivery with you on
          WhatsApp.
        </p>

        <ExternalCta
          href={createWhatsAppCustomLink(
